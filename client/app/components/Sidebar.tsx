        ...
        <ul className="text-sm tracking-wider">
          {sideBarLinks.map((link) => (
            <Link
              key={link.name}
              href={link.link}
              className={`flex items-center my-5 gap-4 px-4 py-3 rounded-full font-medium ${
                link.link === pathname ? "bg-slate-700" : ""
              }`}
            >
              <Image width={20} height={20} src={link.icon} alt={link.name} />
              <p>{link.name}</p>
            </Link>
          ))}
        </ul>
